...
class SportSessionRepositoryImpl @Inject constructor(
    ...
    override suspend fun setSportSession(
        todaySummaryId: String?,
        sportId: String,
        distance: Long,
        duration: Long,
        breakTime: Long,
        goalParameter: SportParameter?,
        sportParameters: List<SportParameter>?
    ) {
        return dataSource.setSportSession(todaySummaryId, sportSessionMapper(sportId, breakTime, goalParameter), sportSessionMapper(distance, duration, sportParameters))
    }
